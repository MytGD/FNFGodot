...
func _init(): effect_callback_type = EFFECT_CALLBACK_TYPE_POST_TRANSPARENT
...
func _check_shader() -> bool:
	...
	mutex.lock()
	if shader_is_dirty: new_shader_code = shader_code; shader_is_dirty = false
	mutex.unlock()

	# We don't have a (new) shader?
	if new_shader_code.is_empty(): return pipeline.is_valid()

	# Apply template.
	new_shader_code = template_shader.replace("#COMPUTE_CODE", get_shader_code());
...
func _render_callback(p_effect_callback_type, p_render_data):
	if not (rd and p_effect_callback_type == EFFECT_CALLBACK_TYPE_POST_TRANSPARENT and _check_shader()): return
